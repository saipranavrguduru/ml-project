...
with $10^{-4}$ consistently better. ViT is an outlier since the higher rate causes a
40-point decrease in micro-F1 when the learning rate is increased during full fine-tuning but has negligible effect
in head-only mode, where fewer parameters are updated.

\textbf{Precision vs.\ recall tradeoff.}
Precision is consistently higher than recall for all models. The models
tend to be conservative and predict less labels than are present rather than
over-predicting. Fully fine-tuning the model improves recall more than precision,
which is why it closes the F1 gap so much more as compared with head-only training.
